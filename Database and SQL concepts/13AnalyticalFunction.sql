...
VALUES ('2021-01-01', 'apple', 10, 1.50),
...
RANK() OVER(ORDER BY units_sold ASC) units_rank,
DENSE_RANK() OVER(ORDER BY units_sold ASC) units_dense_rank,
ROW_NUMBER() OVER(ORDER BY units_sold ASC) units_row_number
FROM sales_data;


SELECT item, units_sold,
LAG(units_sold) OVER(ORDER BY date) as previous_day_unit,
LEAD(units_sold) OVER(ORDER BY date) as next_day_unit
FROM sales_data;
